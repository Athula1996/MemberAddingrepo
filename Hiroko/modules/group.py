...
@Zebra.on_message(filters.command("removephoto")& admin_filter)
async def deletechatphoto(_, message):
      
      chat_id = message.chat.id
      user_id = message.from_user.id
      msg = await message.reply_text("ᴘʀᴏᴄᴇssɪɴɢ....")
      admin_check = await Zebra.get_chat_member(chat_id, user_id)
      if message.chat.type == enums.ChatType.PRIVATE:
           await msg.edit("`ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴡᴏʀᴋ ᴏɴ ɢʀᴏᴜᴘs !`") 
      try:
         if admin_check.privileges.can_change_info:
             await Zebra.delete_chat_photo(chat_id)
...
@Zebra.on_message(filters.command("setphoto")& admin_filter)
...
@Zebra.on_message(filters.command("settitle")& admin_filter)
